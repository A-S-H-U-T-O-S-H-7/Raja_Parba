...
export default function RajaKumariTab() {
  ...
  const { loading, rajaKumari, fetchRajaKumari, updateItemStatus, deleteItem } = useRajaActivityStore();
  ...
  const totalPages = Math.max(1, Math.ceil((rajaKumari?.length || 0) / itemsPerPage));
  const paginatedRajaKumari = useMemo(() => {
    const startIndex = (currentPage - 1) * itemsPerPage;
    return (rajaKumari || []).slice(startIndex, startIndex + itemsPerPage);
  }, [rajaKumari, currentPage]);
  ...
  const handleExport = () => {
    const excelData = buildExcelData(rajaKumari || [], exportColumns);
    exportToExcel(excelData, 'raja-kumari.xls', {
  ...
  if (loading && rajaKumari.length === 0) {
    return (
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full">
            ...
            <tbody className={isDarkMode ? 'divide-y divide-gray-800' : 'divide-y divide-gray-200'}>
              {paginatedRajaKumari.map((item, index) => {
                ...
                return (
                  <tr key={item.id} className={`border-b ${isDarkMode ? 'border-gray-700 hover:bg-gray-800/80' : 'border-gray-300 hover:bg-gray-50'}`}>
                    ...
                    <td className={`px-4 py-4 border-r ${cellBorderClass}`}>
                      <div className="h-16 w-16 overflow-hidden rounded-lg border border-pink-200 bg-gray-100">
                        {item.photoUrl ? (
                    ...
                    <td className={`px-4 py-4 border-r ${cellBorderClass}`}>
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Raja Kumari</p>
                    ...
                    <td className={`px-4 py-4 text-sm border-r ${cellBorderClass} ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                      <p className="flex items-center gap-2"><Mail className="h-4 w-4 text-pink-500" /> {item.email || 'N/A'}</p>
                      <p className="mt-1 flex items-center gap-2"><Phone className="h-4 w-4 text-pink-500" /> {item.phone || 'N/A'}</p>
                    </td>
                    ...
                    <td className="px-4 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        {admin?.role === 'super_admin' && (
                          <button
                            type="button"
                            onClick={() => deleteItem('kumari', item.id)}
          ...
          return (
            <div key={item.id} className={`rounded-2xl border p-4 ${isDarkMode ? 'border-gray-700 bg-gray-900 text-gray-100' : 'border-gray-200 bg-white text-gray-800'}`}>
              ...
              <div className="space-y-1 text-sm">
                <p className="flex items-center gap-2"><Mail className="h-4 w-4 text-pink-500" /> {item.email || 'N/A'}</p>
                <p className="flex items-center gap-2"><Phone className="h-4 w-4 text-pink-500" /> {item.phone || 'N/A'}</p>
      ...
      {rajaKumari.length > itemsPerPage && (
        <Pagination
          currentPage={currentPage}
          totalPages={totalPages}
          totalItems={rajaKumari.length}
      ...
      {rajaKumari.length === 0 && !loading && (
        <div className={`rounded-2xl border p-10 text-center ${isDarkMode ? 'border-gray-700 bg-gray-900 text-gray-400' : 'border-gray-200 bg-white text-gray-500'}`}>
          ...
          <p className="text-lg font-medium">No Raja Kumari applications yet.</p>
        </div>
      )}
      ...
      {editingItem && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/50 p-4">
          <div className={`w-full max-w-lg rounded-2xl border p-6 shadow-2xl ${isDarkMode ? 'border-gray-700 bg-gray-900' : 'border-gray-200 bg-white'}`}>
            <h3 className={`text-xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Update Raja Kumari Status</h3>
            ...
            <div className="mt-6 flex justify-end gap-3">
              ...
              <button
                ...
                onClick={handleSave}
                disabled={saving}
                className="rounded-xl bg-pink-600 px-4 py-2 text-sm font-semibold text-white hover:bg-pink-700 disabled:cursor-not-allowed disabled:bg-pink-400"
              >
