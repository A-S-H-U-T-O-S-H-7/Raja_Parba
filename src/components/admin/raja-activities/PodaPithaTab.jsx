// components/admin/raja-activity/PodaPithaTab.jsx
"use client";
import { useEffect, useMemo, useState } from 'react';
import { Calendar, CheckCircle, Clock, Edit, Mail, Phone, Trash2, XCircle, Cake } from 'lucide-react';
...
export default function PodaPithaTab() {
  const { isDarkMode } = useThemeStore();
  const { admin } = useAdminAuthStore();
  const { loading, podaPitha, fetchPodaPitha, updateItemStatus, deleteItem } = useRajaActivityStore();
  ...
  const totalPages = Math.max(1, Math.ceil((podaPitha?.length || 0) / itemsPerPage));
  const paginatedPodaPitha = useMemo(() => {
    const startIndex = (currentPage - 1) * itemsPerPage;
    return (podaPitha || []).slice(startIndex, startIndex + itemsPerPage);
  }, [podaPitha, currentPage]);
  ...
  const exportColumns = useMemo(() => ([
    ...
    { header: 'Round Type', accessor: (item) => item.roundType || 'Single physical round' },
  ...
  const handleExport = () => {
    const excelData = buildExcelData(podaPitha || [], exportColumns);
    exportToExcel(excelData, 'raja-poda-pitha.xls', {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className={`hidden overflow-hidden rounded-2xl border lg:block ${isDarkMode ? 'border-gray-700 bg-gray-900' : 'border-gray-200 bg-white'}`}>
        <div className="overflow-x-auto">
          <table className="min-w-full">
            <thead className={isDarkMode ? 'bg-gradient-to-r from-amber-950 via-orange-900 to-yellow-900 text-amber-100' : 'bg-gradient-to-r from-amber-50 via-orange-50 to-yellow-100 text-amber-900'}>
              <tr className="text-left text-xs font-bold uppercase tracking-wider">
                ...
                <th className={`px-4 py-3 border-r ${cellBorderClass}`}>Participant</th>
            ...
            <tbody className={isDarkMode ? 'divide-y divide-gray-800' : 'divide-y divide-gray-200'}>
              {paginatedPodaPitha.map((item, index) => {
                ...
                return (
                  <tr key={item.id} className={`border-b ${isDarkMode ? 'border-gray-700 hover:bg-gray-800/80' : 'border-gray-300 hover:bg-gray-50'}`}>
                    ...
                    <td className={`px-4 py-4 border-r ${cellBorderClass}`}>
                      <div className="h-16 w-16 overflow-hidden rounded-lg border border-amber-200 bg-gray-100">
                        {item.photoUrl ? (
                          <img src={item.photoUrl} alt={item.name || 'participant'} className="h-full w-full object-cover" />
                        ) : (
                    ...
                    <td className={`px-4 py-4 border-r ${cellBorderClass}`}>
                      ...
                      <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Poda Pitha</p>
                    ...
                    <td className={`px-4 py-4 text-sm border-r ${cellBorderClass} ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                      <p className="flex items-center gap-2"><Mail className="h-4 w-4 text-amber-500" /> {item.email || 'N/A'}</p>
                      <p className="mt-1 flex items-center gap-2"><Phone className="h-4 w-4 text-amber-500" /> {item.phone || 'N/A'}</p>
                    </td>
                    <td className={`px-4 py-4 text-sm leading-6 border-r ${cellBorderClass} ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                      ...
                      <p>Gender: {item.gender || 'N/A'} | Round: {item.roundType || 'Single physical round'}</p>
                      <p>Category: {item.category || 'Open to all ages'}</p>
                    ...
                    <td className="px-4 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        {admin?.role === 'super_admin' && (
                          <button
                            type="button"
                            onClick={() => deleteItem('podaPitha', item.id)}
          ...
          return (
            <div key={item.id} className={`rounded-2xl border p-4 ${isDarkMode ? 'border-gray-700 bg-gray-900 text-gray-100' : 'border-gray-200 bg-white text-gray-800'}`}>
              ...
              <div className="space-y-1 text-sm">
                <p className="flex items-center gap-2"><Mail className="h-4 w-4 text-amber-500" /> {item.email || 'N/A'}</p>
                <p className="flex items-center gap-2"><Phone className="h-4 w-4 text-amber-500" /> {item.phone || 'N/A'}</p>
                <p>Location: {item.location || 'N/A'}</p>
                <p>Age: {item.age || 'N/A'} | DOB: {item.dob || 'N/A'}</p>
                <p>Round: {item.roundType || 'Single physical round'}</p>
      ...
      {podaPitha.length > itemsPerPage && (
        <Pagination
          currentPage={currentPage}
          totalPages={totalPages}
          totalItems={podaPitha.length}
      ...
      {podaPitha.length === 0 && !loading && (
        <div className={`rounded-2xl border p-10 text-center ${isDarkMode ? 'border-gray-700 bg-gray-900 text-gray-400' : 'border-gray-200 bg-white text-gray-500'}`}>
          <Cake className="mx-auto mb-3 h-12 w-12 opacity-50" />
          <p className="text-lg font-medium">No Poda Pitha applications yet.</p>
        </div>
      )}
      ...
      {editingItem && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/50 p-4">
          <div className={`w-full max-w-lg rounded-2xl border p-6 shadow-2xl ${isDarkMode ? 'border-gray-700 bg-gray-900' : 'border-gray-200 bg-white'}`}>
            <h3 className={`text-xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Update Poda Pitha Status</h3>
            <p className={`mt-1 text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
              {editingItem.name || 'Participant'} ({editingItem.registrationId || editingItem.id})
            </p>
