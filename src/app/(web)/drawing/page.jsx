...
export default function DrawingPage() {
      ...
      await Swal.fire({
        html: `
          <div style="display:flex;flex-direction:column;align-items:center;gap:10px;padding:8px 4px;">
            <div style="width:56px;height:56px;border-radius:9999px;background:linear-gradient(135deg,#10b981,#14b8a6);display:flex;align-items:center;justify-content:center;color:white;font-size:28px;font-weight:700;">&#10003;</div>
            ...
            <p style="margin:0;font-size:0.95rem;color:#4b5563;text-align:center;line-height:1.45;">
              Your Drawing competition registration has been received successfully.
            </p>
            <p style="margin:0;font-size:0.85rem;font-weight:600;color:#10b981;">ID: ${result?.registrationId || result?.id || "Generating..."}</p>
          </div>
        `,
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-emerald-50 via-green-50 to-teal-50 py-6 px-3 md:py-8 md:px-4">
      {/* Decorative Elements */}
      <div className="fixed top-0 left-0 w-48 h-48 bg-emerald-200/30 rounded-full blur-3xl -z-10"></div>
      <div className="fixed bottom-0 right-0 w-64 h-64 bg-teal-200/30 rounded-full blur-3xl -z-10"></div>
      
      <div className="max-w-4xl mx-auto">
        {/* Header with Festival Theme */}
        <div className="text-center mb-6">
          <div className="inline-flex items-center gap-2 bg-white/80 backdrop-blur-sm px-4 py-1.5 rounded-full shadow-sm border border-emerald-200 mb-3">
            <Flower2 className="w-3.5 h-3.5 text-emerald-500" />
            <span className="text-xs font-medium text-emerald-600">Raja Parba 2026</span>
            <Flower2 className="w-3.5 h-3.5 text-emerald-500" />
          </div>
          <h1 className="text-2xl md:text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-emerald-600 to-teal-600">
            Drawing Competition
          </h1>
        ...
        {/* Main Form Card */}
        <div className="relative bg-white/95 backdrop-blur-sm rounded-2xl shadow-lg border border-emerald-200 overflow-hidden">
          {/* Decorative Top Border */}
          <div className="h-1.5 bg-gradient-to-r from-emerald-400 via-green-500 to-teal-500"></div>
          
          {/* Title Section with Soft Gradient Background */}
          <div className="bg-gradient-to-r from-emerald-50 to-teal-50 px-5 py-4 border-b border-emerald-200">
            ...
            <div className="bg-gradient-to-r from-emerald-50 to-teal-50 p-4 rounded-xl border border-emerald-200">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                {competitionItems.map((item, index) => (
                  <div
                    key={item}
                    className="relative bg-white rounded-lg border border-emerald-200 px-3 py-2 text-center group hover:border-emerald-300 transition-all"
                  >
                    <span className="absolute -top-2 -left-2 w-5 h-5 bg-gradient-to-br from-emerald-500 to-teal-600 rounded-full flex items-center justify-center">
          ...
          <form onSubmit={onSubmit} className="p-5 md:p-6">
            ...
            <div className="mb-6">
              ...
              <div className="relative">
                {photoPreview ? (
                  <div className="relative w-full h-32 rounded-xl overflow-hidden border border-emerald-200">
                    <img 
                      src={photoPreview} 
                      alt="Preview" 
                  ...
                  <label className="relative flex items-center justify-center w-full h-32 border-2 border-dashed border-emerald-300 rounded-xl bg-emerald-50 cursor-pointer hover:bg-emerald-100 transition group">
                    <div className="text-center text-gray-600">
                      <Camera className="w-6 h-6 mx-auto mb-1 text-emerald-400 group-hover:text-emerald-600" />
            ...
            <div className="mb-6">
              ...
              <div className="grid grid-cols-2 gap-3">
                <label className={`relative rounded-lg border px-4 py-3 cursor-pointer transition-all ${
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full rounded-lg border border-emerald-200 bg-white py-2.5 pl-9 pr-3 text-sm text-gray-700 outline-none focus:border-emerald-400 focus:ring-1 focus:ring-emerald-100 transition-all"
                  required
                />
              </div>
              ...
              {/* Gender Dropdown */}
              <div className="relative">
                <User className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-emerald-400 z-10" />
                <select
                  value={form.gender}
                  onChange={(e) => updateField("gender", e.target.value)}
                  className="w-full rounded-lg border border-emerald-200 bg-white py-2.5 pl-9 pr-8 text-sm text-gray-700 outline-none focus:border-emerald-400 focus:ring-1 focus:ring-emerald-100 transition-all appearance-none"
                  required
                >
              ...
              {/* Age (Auto-calculated) */}
              <div className="relative">
                <Cake className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-emerald-400" />
                <input
                  type="text"
                  placeholder="Age (auto calculated)"
                  value={form.age}
                  disabled
                  className="w-full rounded-lg border border-emerald-200 bg-emerald-50 py-2.5 pl-9 pr-3 text-sm text-gray-600 cursor-not-allowed"
                />
              </div>
            </div>

            {/* Submit Button - Centered */}
            <div className="flex justify-center pt-6 mt-2 border-t border-emerald-200">
              <button
                type="submit"
                disabled={submitting}
                className="group cursor-pointer relative px-8 py-2.5 bg-gradient-to-r from-emerald-500 via-green-500 to-teal-500 text-white font-medium text-sm rounded-lg shadow-md hover:shadow-lg transition-all duration-300 disabled:opacity-60 disabled:cursor-not-allowed overflow-hidden"
              >
                <span className="relative z-10 flex items-center gap-1.5">
                  {submitting ? (
                    ...
                    <>
                      <Palette className="w-4 h-4" />
                      <span>Register for Drawing Competition</span>
        ...
        <p className="text-center text-xs text-gray-500 mt-4 flex items-center justify-center gap-1">
          <Flower2 className="w-3 h-3 text-emerald-400" />
